...
try:
    ...
    for i, csv_file in enumerate(csv_files):
        ...
        collection_name = ticker_name.replace('.', '_').replace('$', '').replace(' ', '_').replace('#', '')
        ...
        try:
            ...
            if documents:
                ...
                num_inserted_docs_current_file = len(result.inserted_ids)
                total_documents_inserted += num_inserted_docs_current_file

                print(f"  Insertados {num_inserted_docs_current_file} documentos en la colección '{collection_name}'.")
...
print(f"Total de documentos (registros de datos) insertados en MongoDB: {total_documents_inserted}")
if failed_files:
